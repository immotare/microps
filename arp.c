...
arp_cache_delete(struct arp_cache *cache)
{
    ...
    debugf("DELTER: pa=%s, ha=%s", ip_addr_ntop(cache->pa, addr1, sizeof(addr1)), ether_addr_ntop(cache->ha, addr2, sizeof(addr2)));
...
arp_reply(struct net_iface *iface, const uint8_t *tha, ip_addr_t tpa, const uint8_t *dst)
{
    ...
    struct ip_iface *ipface;
    ipface = (struct ip_iface*)iface;
    ...
    memcpy(&reply.spa, &ipface->unicast, sizeof(reply.spa));
...
arp_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (ntoh16(msg->hdr.hrd) != ARP_HRD_ETHER || msg->hdr.hln != ETHER_ADDR_LEN) {
        errorf("hw address type is not ethernet");
        return;
    }


    if (ntoh16(msg->hdr.pro) != ARP_PRO_IP || msg->hdr.pln != IP_ADDR_LEN) {
        errorf("pro address type is not ip");
        return;
    }
    ...
    if (iface && ((struct ip_iface *)iface)->unicast == tpa) {
        ...
        if (ntoh16(msg->hdr.op) == ARP_OP_REQUEST) {
            arp_reply(iface, msg->sha, spa, (const uint8_t *)msg->sha);
        }
    }
}
...
arp_resolve(struct net_iface *iface, ip_addr_t pa, uint8_t *ha)
{
    ...
    if (!cache) {
        cache = arp_cache_alloc();
        if (!cache)return ARP_RESOLVE_ERROR;
...
arp_timer_handler(void)
{
    ...
    for (entry = caches; entry < tailof(caches); entry++) {
        if (entry->state != ARP_CACHE_STATE_FREE && entry->state != ARP_CACHE_STATE_STATIC) {
            ...
            struct timeval arp_cache_timeout = {ARP_CACHE_TIMEOUT, 0};
            if (timercmp(&diff, &arp_cache_timeout, >) != 0) {
...
int arp_init(void)
{
    ...
    if (net_timer_register(interval, arp_timer_handler) == -1) {
        errorf("net_timer_register() falure");
        return -1;
    }
    return 0;
}