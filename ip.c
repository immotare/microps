...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    if (src == IP_ADDR_ANY) {
        ...
        if (!iface) {
            errorf("interface with src address not found");
            return -1;
        }

        ip_addr_t iface_net_addr = (iface->unicast) &  (iface->netmask);

        if (!((dst & iface->netmask) == iface_net_addr || dst == IP_ADDR_BROADCAST)) {
            errorf("dst is unreachable");
            return -1;
        }
    }
