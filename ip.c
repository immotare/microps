...
ip_route_add(ip_addr_t network, ip_addr_t netmask, ip_addr_t nexthop, struct ip_iface *iface)
{
    ...
    infof("route added: network=%s, netmask=%s, nexthop=%s, iface=%s, dev=%s", ip_addr_ntop(route->network, addr1, sizeof(addr1)), ip_addr_ntop(route->netmask, addr2, sizeof(addr2)), ip_addr_ntop(route->nexthop, addr3, sizeof(addr3)), ip_addr_ntop(route->iface->unicast, addr4, sizeof(addr4)), NET_IFACE(iface)->dev->name);
    return route;
}
...
ip_iface_alloc(const char *unicast, const char *netmask)
{
    ...
    if (ip_addr_pton(unicast, &iface->unicast) == -1) {
        memory_free(iface);
        errorf("convert unicast to binary failure");
        return NULL;
    }

    if(ip_addr_pton(netmask, &iface->netmask) == -1) {
        memory_free(iface);
        errorf("convert netmask to binary failure");
        return NULL;
    }

    ip_addr_t net_address = (iface->unicast) & (iface->netmask);
    iface->broadcast = net_address | ~(iface->netmask);
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    hlen = (hdr->vhl & 0x0f) << 2;
    if (len < hlen) {
        errorf("len is shorter than hlen");
        return;
    }

    total = ntoh16(hdr->total);
    if (len < total) {
        errorf("len is shorter than total");
        return;
    }

    if (cksum16((uint16_t *)hdr, IP_HDR_SIZE_MIN, 0) != 0) {
    ...
    for (protocol = protocols; protocol; protocol = protocol->next) {
        if (protocol->type == hdr->protocol) {
            protocol->handler((const uint8_t *)hdr+hlen, total-hlen, hdr->src, hdr->dst, iface);
            return;
        }
    }
}
...
ip_output_core(struct ip_iface *iface, uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, ip_addr_t nexthop, uint16_t id, uint16_t offset)
{
    ...
    hdr->src = src; // testでネットワークオーダに変換済み
    hdr->dst = dst; // testでネットワークオーダに変換済み
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    if (src == IP_ADDR_ANY && dst == IP_ADDR_BROADCAST) {
        errorf("source address is required for broad cast addresses");
        return -1;
    }
    route = ip_route_lookup(dst);
    if (!route) {
    ...
    debugf("protocol=%u, len=%u, dst=%s", protocol, len, ip_addr_ntop(dst, addr, sizeof(addr)));
    return len;
}
